...
function AnimatedBackground({ theme }) {
    ...
    const interval = setInterval(() => {
      setDots(prevDots =>
        prevDots.map(dot => ({
          ...
          opacity: Math.abs(Math.sin(Date.now() * 0.001 * dot.twinkleSpeed)) * 0.7 + 0.3,
        }))
      );
    }, 150);
  ...
  const getOverlayGradient = () => {
    if (theme === 'green') {
      return "bg-gradient-to-tr from-emerald-950/15 via-transparent to-green-950/10";
    } else {
      return "bg-gradient-to-tr from-blue-950/15 via-transparent to-indigo-950/10";
    }
  };
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
      ...
      {dots.map(dot => (
        <div
          ...
          style={{
            ...
            height: `${dot.size}px`,
            opacity: dot.opacity,
            background: `radial-gradient(circle, ${dotColor.color} 0%, ${dotColor.color.replace('1)', '0.8)')} 30%, transparent 70%)`,
            boxShadow: `
      ...
      <div className={`absolute top-1/4 right-1/3 w-96 h-96 ${theme === 'green' ? 'bg-green-500/2' : 'bg-blue-500/2'} rounded-full blur-3xl`}></div>
      <div className={`absolute bottom-1/3 left-1/4 w-64 h-64 ${theme === 'green' ? 'bg-emerald-400/2' : 'bg-indigo-400/2'} rounded-full blur-2xl`}></div>
    </div>
  );
}
